...
@section('content')
    ...
    <form action="{{route('contact-update-submit', $data->id)}}" method="post">
        ...
        <div class="form-group">
            ...
            <textarea type="message" name="message" id="message" class="form-control" placeholder="Введите сообщение" >{{$data->message}}</textarea>
        </div>

        <div class="form-group">
            <label for="status_active">status_active</label>
            <input type="text" name="status_active" value="{{$data->status_active}}" placeholder="1 по умолчанию или 0" id="status_active" class="form-control">
        </div>
